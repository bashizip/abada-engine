...
public class TaskControllerTest {
    ...
    @Test
    @DisplayName(
        "Should execute full task lifecycle with correct status and date transitions"
    )
    void shouldExecuteFullTaskLifecycleViaApi() {
        ...
        ResponseEntity<List<TaskDetailsDto>> listResponse =
        ...
        assertThat(initialTask.processDefinitionId()).isEqualTo("recipe-cook");
        assertThat(initialTask.processDefinitionName()).isEqualTo(
            "recipe-cook"
        );
        assertThat(initialTask.processStatus()).isEqualTo(
            ProcessStatus.RUNNING
        );
        assertThat(initialTask.processSuspended()).isFalse();
        assertThat(initialTask.processStartDate()).isNotNull();
        assertThat(initialTask.currentActivityId()).isNotNull();
        ...
        assertThat(detailsResponse.getBody().processDefinitionId()).isEqualTo(
            "recipe-cook"
        );
        assertThat(detailsResponse.getBody().processStatus()).isEqualTo(
            ProcessStatus.RUNNING
        );
        assertThat(detailsResponse.getBody().variables()).isNotNull();

        // --- Step 4: Alice claims the task ---
        restTemplate.exchange(
            "/v1/tasks/claim?taskId={taskId}",
            HttpMethod.POST,
            aliceRequestEntity,
            Map.class,
            taskId
        ...
        ResponseEntity<TaskDetailsDto> completedDetailsResponse =
            restTemplate.exchange(
                "/v1/tasks/{id}",
                HttpMethod.GET,
                aliceRequestEntity,
        ...
        assertThat(completedDetailsResponse.getBody().status()).isEqualTo(
        ...
        ResponseEntity<List<TaskDetailsDto>> bobListResponse =
            restTemplate.exchange(
                "/v1/tasks",
                HttpMethod.GET,
                bobRequestEntity,
    ...
    void shouldFilterTasksByStatus() {
        ...
        ResponseEntity<List<TaskDetailsDto>> availableResponse =
            restTemplate.exchange(
                "/v1/tasks?status=AVAILABLE",
                HttpMethod.GET,
                aliceRequestEntity,
                new ParameterizedTypeReference<>() {}
            );
        ...
        ResponseEntity<List<TaskDetailsDto>> claimedEmptyResponse =
            restTemplate.exchange(
                "/v1/tasks?status=CLAIMED",
                HttpMethod.GET,
                aliceRequestEntity,
                new ParameterizedTypeReference<>() {}
            );
        ...
        ResponseEntity<List<TaskDetailsDto>> claimedResponse =
            restTemplate.exchange(
                "/v1/tasks?status=CLAIMED",
                HttpMethod.GET,
                aliceRequestEntity,
        ...
        ResponseEntity<List<TaskDetailsDto>> availableEmptyResponse =
            restTemplate.exchange(
                "/v1/tasks?status=AVAILABLE",
                HttpMethod.GET,
                aliceRequestEntity,
    ...
    void shouldFailTask() {
        ...
        assertThat(detailsResponse.getBody().status()).isEqualTo(
            TaskStatus.FAILED
        );
        assertThat(detailsResponse.getBody().startDate()).isNotNull();
    ...
    @Test
    @DisplayName(
        "Should return 400 error when claiming an already claimed task"
    )
    void shouldReturnErrorWhenClaimingUnavailableTask() {
        ...
        assertThat(errorResponse.getBody().message()).contains(
            "Task is not available to be claimed"
        );
    }

    @Test
    @DisplayName(
        "Should return 400 error when completing a task assigned to another user"
    )
    void shouldReturnErrorWhenCompletingUnassignedTask() {
        ...
        bobHeaders.set("X-User", "bob");
        bobHeaders.set("X-Groups", "customers");
        HttpEntity<Map<String, Object>> bobCompleteRequest = new HttpEntity<>(
            Map.of("goodOne", true),
            bobHeaders
        );
        ResponseEntity<ErrorResponse> errorResponse = restTemplate.exchange(
    ...
    @Test
    @DisplayName(
        "GET /v1/tasks/user-stats should return comprehensive user statistics"
    )
    void shouldReturnUserStats() {
        ...
        assertThat(stats.quickStats().completedTasks()).isGreaterThanOrEqualTo(
            1
        ); // At least 1 COMPLETED task
        assertThat(
            stats.quickStats().runningProcesses()
        ).isGreaterThanOrEqualTo(3); // At least 3 running processes
        assertThat(stats.quickStats().availableTasks()).isGreaterThanOrEqualTo(
            1
        ); // At least 1 AVAILABLE task
        ...
        assertThat(stats.recentTasks().get(0).name()).isEqualTo(
            "Choose Recipe"
        );
        ...
        assertThat(
            stats.tasksByStatus().get(TaskStatus.CLAIMED)
        ).isGreaterThanOrEqualTo(1);
        assertThat(
            stats.tasksByStatus().get(TaskStatus.COMPLETED)
        ).isGreaterThanOrEqualTo(1);
        ...
        assertThat(
            stats.processActivity().recentlyStartedProcesses()
        ).isNotNull();
        assertThat(
            stats.processActivity().recentlyStartedProcesses()
        ).hasSizeGreaterThanOrEqualTo(3); // At least 3 processes
        assertThat(
            stats.processActivity().activeProcessCount()
        ).isGreaterThanOrEqualTo(3); // At least 3 active processes
        assertThat(
            stats.processActivity().completionRate()
        ).isGreaterThanOrEqualTo(0.0); // Completion rate >= 0
    }

    @Test
    @DisplayName(
        "GET /v1/tasks/user-stats should return empty stats for user with no tasks"
    )
    void shouldReturnEmptyStatsForUserWithNoTasks() {
        ...
        HttpEntity<Void> charlieRequestEntity = new HttpEntity<>(
            charlieHeaders
        );
