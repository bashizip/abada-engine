...
import { LayoutDashboard, List, AlertTriangle, BarChart2, Bell, Settings, User } from 'lucide-react';
    ...
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <header className="sticky top-0 z-40 w-full border-b border-slate-700 bg-slate-900/90 backdrop-blur supports-[backdrop-filter]:bg-slate-900/60">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <NavLink to="/" className="flex items-center gap-2 font-bold text-xl tracking-tight text-slate-100">
            ...
            <nav className="hidden md:flex items-center gap-1">
              ...
              <NavItem to="/instances" icon={<List size={18} />}>Instances</NavItem>
              <NavItem to="/jobs" icon={<AlertTriangle size={18} />}>Jobs</NavItem>
              <NavItem to="/metrics" icon={<BarChart2 size={18} />}>Metrics</NavItem>
            </nav>
          </div>
      ...
      <footer className="border-t border-slate-800 py-6 mt-8">
        <div className="container mx-auto px-4 text-center text-slate-500 text-sm">
          &copy; {new Date().getFullYear()} Abada BPMN Engine Operations. All rights reserved.
        </div>
      </footer>
    </div>
  );
